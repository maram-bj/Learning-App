...
public struct ChangeLearningGoalView: View {
    ...
    public var body: some View {
        VStack(alignment: .leading, spacing: 40) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                HStack(spacing: 12) {
                    durationButton(title: "Week", type: .weekly)
                    durationButton(title: "Month", type: .monthly)
                    durationButton(title: "Year", type: .yearly)
                }
            }
    ...
    @ViewBuilder
    private func durationButton(title: String, type: DurationType) -> some View {
        Button {
            withAnimation(.easeInOut(duration: 0.2)) {
                viewModel.onDurationSelected(type)
            }
        } label: {
            ZStack {
                RoundedRectangle(cornerRadius: 34)
                    .fill(viewModel.selectedDuration == type ? Color.orange : Color.white.opacity(0.08))
                    .frame(height: 48)
                    .shadow(
                        color: viewModel.selectedDuration == type ? Color.orange.opacity(0.6) : .clear,
                        radius: 6,
                        y: 3
                    )
                ...
                Text(title)
                    ...
                    .foregroundColor(viewModel.selectedDuration == type ? .white : .gray)
            }
        }
        .buttonStyle(.plain)
    }
